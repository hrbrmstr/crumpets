#' WIP - Check if a user is a collaborator of a repository
#'
#' @md
...
#' @examples \dontrun{
#' }
repo_check_collaborator <- function(owner, repo, collaborator, api_endpoint = Sys.getenv("GITEA_BASE_URL"),
                                    gitea_token = Sys.getenv("GITEA_PAT")) {
  stop("Not implemented yet")

  api_endpoint <- sub("/$", "", api_endpoint)

  gitea_url <- file.path(api_endpoint, "api/v1", sub("^/", "", "/repos/{owner}/{repo}/collaborators/{collaborator}"))
