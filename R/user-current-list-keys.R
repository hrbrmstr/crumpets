#' WIP - List the authenticated user's public keys
#'
#' @md
...
#' @examples \dontrun{
#' }
user_current_list_keys <- function(api_endpoint = Sys.getenv("GITEA_BASE_URL"),
                                   gitea_token = Sys.getenv("GITEA_PAT")) {
  stop("Not implemented yet")

  api_endpoint <- sub("/$", "", api_endpoint)

  gitea_url <- file.path(api_endpoint, "api/v1", sub("^/", "", "/user/keys"))
