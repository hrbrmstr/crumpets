#' WIP - List a repository's issues
#'
#' @md
...
#' @param page page number of requested issues
...
#' @examples \dontrun{
#' }
issue_list_issues <- function(owner, repo, state, page, api_endpoint = Sys.getenv("GITEA_BASE_URL"),
                              gitea_token = Sys.getenv("GITEA_PAT")) {
  stop("Not implemented yet")

  api_endpoint <- sub("/$", "", api_endpoint)

  gitea_url <- file.path(api_endpoint, "api/v1", sub("^/", "", "/repos/{owner}/{repo}/issues"))
