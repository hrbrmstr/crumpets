#' WIP - Check if one user is following another user
#'
#' @md
...
#' @examples \dontrun{
#' }
user_check_following <- function(follower, followee, api_endpoint = Sys.getenv("GITEA_BASE_URL"),
                                 gitea_token = Sys.getenv("GITEA_PAT")) {
  stop("Not implemented yet")

  api_endpoint <- sub("/$", "", api_endpoint)

  gitea_url <- file.path(api_endpoint, "api/v1", sub("^/", "", "/users/{follower}/following/{followee}"))
